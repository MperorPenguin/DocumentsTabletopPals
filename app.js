/* DM Toolkit — World Map + DM Page + Dice + Live Sync + Better Image Handling
   - Loads JSON from MapsKit/assets/maps/index.json (or index.js under file://)
   - Supports multiple maps; auto-pretty names from filenames
   - World view uses a scrollable "stage" with Fit/Actual (dbl-click)
   - Fullscreen: Actual size, scrollable
   - Pop-out: toolbar with Fit / Actual / Zoom+ / Zoom− / Reset / Full
   - Dice panel + DM Tools Quick Dice (animated)
*/
...
function prettyNameFromPath(p){
  ...
  // Preserve your chosen casing; just replace separators with spaces
...
function normalizeMapsList(list){
  ...
  // Optional: sort by name for nicer lists
...
function renderWorld(){
  ...
  // Double-click to toggle Fit/Actual in-page
...
function toggleWorldFullscreen(){
  ...
  const el = stage;
  // Enter/exit fullscreen on the stage wrapper
  const enter = !document.fullscreenElement;
  if(enter){
    // go fullscreen and show actual size (scrollable)
    stage.classList.remove('fit'); stage.classList.add('actual');
...
// cleanup stage class when fullscreen changes
document.addEventListener('fullscreenchange', ()=>{
  const stage = getWorldStage();
  if(!stage) return;
  if(!document.fullscreenElement){
    // Leave fullscreen: restore preferred mode
...
function openWorldViewer(){
  ...
  <style>
    ...
    .btn{appearance:none;background:#0f1115;border:1px solid #23283a;color:#e6e9f2;padding:6px 10px;border-radius:10px;cursor:pointer}
    .btn:hover{border-color:#A78BFA;box-shadow:0 0 0 2px rgba(167,139,250,.3) inset}

    /* Stage holds a canvas that always centers the image */
    .stage{position:fixed;inset:0;top:50px;overflow:auto;background:#0b1020}
    ...
    <script>
      ...
      function reset(){
        ...
        applyScale();
        setMode('fit');
        // center scroll to middle if image is smaller than viewport
...
function quickAdvFor(kind,id){
  ...
  if(t) showToast('Advantage', `${t.name} (${t.cls})`);
}
function quickDisFor(kind,id){
  const t = findToken(kind,id);
  if(t) showToast('Disadvantage', `${t.name} (${t.cls})`);
}
...
function renderDmPage(){
  ...
  if(tab==='tools'){
    ...
    body+=`
      ...
      <!-- Quick Dice (same animated behavior as Dice page) -->
      <div class="dm-section" style="margin-top:10px">
...
function showToast(title,msg){
  ...
  activeToast = div;
  setTimeout(()=>{ if(div===activeToast){ div.remove(); activeToast=null; } }, 3200);
}
